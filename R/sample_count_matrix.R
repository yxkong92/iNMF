...
#' @param N An integer. The number of genes.
...
#' @param Nm An integer. The number of marker genes in one cell type.
#' @param Na An integer. The number of anti-marker genes in one cell type.
#' @param Nh An integer. The number of house-keeping genes in one cell type.
...
#' list1<-samplecounts(N=10,M1=5,L=0,Ks=0,Kb=2,Nm=2,Na=0,Nh=5,alpha=c(1,1,1),missing="logit")
#' X<-list1$X
#' trueA<-list1$A
#' trueW<-list1$W
#' #generate a count matrix X(bulk counts) with single cell counts.
#' list2<-samplecounts(N=15,M1=5,Ks=3,Kb=3,Nm=2,Na=0,Nh=5,alpha=c(1,2,3),missing="logit")
#' X<-list2$X
...
samplecounts<-function(N,M1,Ks,Kb,Nm,Na,Nh,alpha,missing="logit",mkappa=1,mtau=1.5){
  ...
  if (Ks==0){
    ...
    w <- matrix(0,nr=K, nc=30*(Ks-1))
    for (i in 2:Ks){
      cl<-numeric(K)
      cl[i]<-1
      w[,(30*(i-2)+1):(30*(i-2)+30)]<-cl
    }
    W2<-cbind(matrix(c(rep(a,40)),ncol = 40),w)
    if (Kb<Ks) {
