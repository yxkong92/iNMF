...
double atermalpha(int n, double x, double t)
{
  double f = 0;
  if(x <= t) {
...
double exprndalpha(double mu)
{
  return -mu * log(1.0 - R::runif(0.0,1.0));
}

double truncgammaalpha()
{
  double c = MATH_PI_2;
  double X, gX;
...
double randinvgalpha(double mu)
{
...
double tinvgaussalpha(double z, double t)
{
  double X, u;
  double mu = 1.0/z;
...
double samplepgalpha(double z)
{
  ...
  while(1) 
  {
    u = R::runif(0.0,1.0);
    if(u < ratio) {
      X = t + exprndalpha(1.0)/K;
    }
    else {
      X = tinvgaussalpha(z, t);
    }
    
    int i = 1;
    double Sn = atermalpha(0, X, t);
...
void computeomegaalpha(NumericVector Kappa, NumericVector Tau, NumericVector probvec, int N,  int M, NumericVector result)
{
  int i;
  int j;
  ...
  for (i=0; i < n; i++){
    for(j=0; j < m; j++){
      result[j * n + i] = samplepgalpha( Kappa[j]  + Tau[j] * probvec[j *n + i]) ; 
    }
  }      
}


void sampleKTalpha(NumericVector Omega, NumericVector S, NumericVector probM, double muKappa, double sigmaKappa, double muTau, double sigmaTau, int N, int M, NumericVector output, NumericVector SOmega, NumericVector SOmegaP, NumericVector SOmegaPsq, NumericVector SS, NumericVector SSP){
...
void sampleSalpha(NumericVector Words, NumericVector probM, NumericVector S, NumericVector Kappa, NumericVector Tau, NumericVector Total, int N, int M, NumericVector output){
...
void testalpha(NumericVector Av, int updateA,NumericVector Wv, NumericVector Xv,  NumericVector Total, NumericVector alpha, NumericVector eqS, NumericVector EqSKappa, NumericVector EqSTau, NumericVector EqKappa, NumericVector EqTau, NumericVector EqOmegaKappasq, NumericVector EqOmegaKappaTau, NumericVector EqOmegaTausq, int N, int M, int M1, int K, NumericVector output){
...
double lfunctionalpha(NumericVector Av,NumericVector Wv, NumericVector Xv, NumericVector Total, NumericVector alpha, NumericVector EqS, NumericVector EqSKappa, NumericVector EqSTau, NumericVector EqKappa, NumericVector EqTau, NumericVector EqOmegaKappasq,NumericVector EqOmegaKappaTau,NumericVector EqOmegaTausq, int N, int M, int K, double sdKappa, double sdTau){
  ...
  for(k=0; k<kk; k++){
    for(j=0; j<m; j++){
      tot = tot + (alpha[k]-1.0)*log(Wv[m*k+j]);
    }
  }
...
Rcpp::NumericVector MCMCsample2alpha(NumericVector Xv, NumericVector Total, int N, int M, int M1, int K, NumericVector Av, int updateA, NumericVector Wv,NumericVector alpha, int Mccount, int burnin, NumericVector EqS, NumericVector EqSKappa, NumericVector EqSTau, NumericVector EqKappa,  NumericVector EqKappasq, NumericVector EqTau, NumericVector EqTausq, NumericVector EqOmegaKappasq, NumericVector EqOmegaKappaTau, NumericVector EqOmegaTausq){
  ...
  while((diff1 > thres) & (outer < 30)){
    ...
    for(miter = 0; miter < burnin-1; miter++){
      computeomegaalpha(Kappa, Tau, prob, N,  M, Omega);                
      sampleKTalpha(Omega, S, prob, muKappa, sdKappa,  muTau, sdTau, N, M, KT, SOmega, SOmegaP, SOmegaPsq, SS, SSP);
      for(j = 0; j < m; j ++){
        Kappa[j] = KT[j];
        Tau[j] = KT[m+j];
      }
      sampleSalpha(Xv, prob, S, Kappa, Tau, Total, N, M, nS);
    ...
    testalpha(Av, updateA, Wv,  Xv, Total, alpha, EqS, EqSKappa, EqSTau, EqKappa, EqTau,EqOmegaKappasq,EqOmegaKappaTau,EqOmegaTausq, N, M,M1, K, output);
    ...
    ob = lfunctionalpha(Av, Wv, Xv, Total, alpha, EqS, EqSKappa, EqSTau, EqKappa, EqTau, EqOmegaKappasq, EqOmegaKappaTau, EqOmegaTausq, N, M,K, sdKappa, sdTau);
